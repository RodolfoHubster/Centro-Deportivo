...
    // Mostrar mensaje de carga (preservando título y descripción)
    main.innerHTML = '';
    if (titulo) main.appendChild(titulo);
    if (descripcion) main.appendChild(descripcion);
    main.innerHTML += '<p style="text-align: center;">Cargando...</p>';
...
        tarjeta.setAttribute('data-tipo-registro', evento.tipo_registro || 'Individual'); // <-- Esto está correcto
        tarjeta.setAttribute('data-integrantes-min', evento.integrantes_min || 8); 
        tarjeta.setAttribute('data-integrantes-max', evento.integrantes_max || 0); //(0 = sin límite)
...
        let cupoInfo = '';
        if (evento.cupo_maximo > 0) { // Solo mostrar si cupo_maximo NO es null o 0
...
            `;
        } else {
            cupoInfo = `<p><strong>Cupo:</strong> Sin límite</p>`;
        }
...
    // Volver a añadir botones de inscripción (de Inscripcion.js)
