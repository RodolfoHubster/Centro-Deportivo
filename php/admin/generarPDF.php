...
try {
    ...
    while ($row = mysqli_fetch_assoc($resultado)) {
        if ($row['es_tronco_comun']) $row['carrera_display'] = "TC - " . $row['area_tronco_comun'];
        else $row['carrera_display'] = $row['carrera_nombre'];
    ...
    // --- SECCIÓN FILTROS APLICADOS (RESTAURADA COMPLETA) ---
    if (!$esReporteEvento) {
    ...
    $pdf->Cell($colWidth, 8, 'Otros', 1, 0, 'C', true);
    ...
    $pdf->Cell($colWidth, 8, $total_otros, 1, 0, 'C', true);
    ...
    $cols = [
        ...
        ['titulo' => 'Nombre', 'campo' => 'nombre_completo', 'w' => 30, 'align' => 'L'],
        ['titulo' => 'Correo', 'campo' => 'correo_institucional', 'w' => 25, 'align' => 'L'],
        ['titulo' => 'Género', 'campo' => 'genero', 'w' => 10, 'align' => 'C'],
        ['titulo' => 'Tipo', 'campo' => 'tipo_participante', 'w' => 13, 'align' => 'C'],
        ['titulo' => 'Carrera', 'campo' => 'carrera_display', 'w' => 20, 'align' => 'L'],
    ];
    ...
    if ($mostrarRol) {
        array_splice($cols, 2, 0, [['titulo' => 'Rol', 'campo' => 'rol_calculado', 'w' => 10, 'align' => 'C']]);
        $cols[1]['w'] = 25; 
    }

    if (!$esReporteEvento) {
        $cols[] = ['titulo' => 'Evento', 'campo' => 'evento_nombre', 'w' => 15, 'align' => 'L'];
        $cols[1]['w'] = 20; 
    }

    // Calcular Anchos
    $totalWeight = 0;
    foreach($cols as $c) $totalWeight += $c['w'];
    $colWidths = [];
    foreach($cols as $k => $c) {
        $colWidths[$k] = ($c['w'] / $totalWeight) * $pageWidth;
    }
    ...
    foreach ($datos as $row) {
        ...
        if (!$esReporteEvento && $tipoActual !== $ultimoTipoRegistro) {
            ...
            $pdf->SetFont('helvetica', 'B', 11);
            $pdf->SetFillColor($color_seccion[0], $color_seccion[1], $color_seccion[2]);
            $pdf->SetTextColor(255, 255, 255);
        ...
        foreach($cols as $k => $c) {
            $valor = $row[$c['campo']] ?? '';
            $pdf->Cell($colWidths[$k], 6, substr($valor, 0, 35), 1, 0, $c['align'], $usarFill);
        }
        $pdf->Ln();
        $fill = !$fill;
    }
