...
if (!$evento_id || !$facultad_id || !in_array($sexo, ['Hombre', 'Mujer'])) {
...
try {
    // 1. Obtener cupos para esta facultad en el evento
    $sqlCupo = "SELECT cupo_hombres, cupo_mujeres 
                FROM evento_facultad_cupos 
                WHERE evento_id = ? AND facultad_id = ?";
    $stmtCupo = mysqli_prepare($conexion, $sqlCupo);
    if (!$stmtCupo) throw new Exception(mysqli_error($conexion));
    mysqli_stmt_bind_param($stmtCupo, 'ii', $evento_id, $facultad_id);
    ...
    if (!$cupos) {
        throw new Exception('La facultad seleccionada no está disponible para esta Pausa Activa.');
    }

    $limite = ($sexo === 'Hombre') ? (int)$cupos['cupo_hombres'] : (int)$cupos['cupo_mujeres'];

    // 2. Contar registros actuales para esa facultad y sexo
    $sqlCount = "SELECT COUNT(*) AS total 
                 FROM inscripcion_pausa_activa 
                 WHERE evento_id = ? AND facultad_id = ? AND sexo = ?";
    $stmtCount = mysqli_prepare($conexion, $sqlCount);
    if (!$stmtCount) throw new Exception(mysqli_error($conexion));
    mysqli_stmt_bind_param($stmtCount, 'iis', $evento_id, $facultad_id, $sexo);
    ...
    if ($limite > 0 && $registrados >= $limite) {
        throw new Exception("Lo sentimos, ya no hay espacios disponibles para {$sexo} en esta facultad.");
    }

    // 4. Insertar registro (anónimo — no requiere usuario_id)
    $sqlInsert = "INSERT INTO inscripcion_pausa_activa (evento_id, facultad_id, sexo, fecha_registro) 
                  VALUES (?, ?, ?, NOW())";
    $stmtInsert = mysqli_prepare($conexion, $sqlInsert);
    if (!$stmtInsert) throw new Exception(mysqli_error($conexion));
    mysqli_stmt_bind_param($stmtInsert, 'iis', $evento_id, $facultad_id, $sexo);
